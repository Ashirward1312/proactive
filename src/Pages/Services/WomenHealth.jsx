...
export default function WomenHealth() {
  ...
  const treatments = [
    ...
    "Safe Postpartum Return to Exercise",
    "Clinical Pilates for Women",
  ...
  const processSteps = [
    {
      ...
      desc: "Compassionate evaluation of pelvic and overall functional health.",
      icon: <Search className="w-5 h-5" />,
      chip: "bg-blue-600"
    },
    {
      title: "Holistic Plan",
      desc: "Tailored strategies for your unique life stage and health goals.",
      icon: <FileText className="w-5 h-5" />,
      chip: "bg-[#C1121F]"
    },
    {
      title: "Gentle Therapy",
      desc: "Clinical techniques to restore core and pelvic floor function.",
      icon: <Heart className="w-5 h-5" />,
      chip: "bg-emerald-600"
    },
    {
      title: "Vital Recovery",
      desc: "Ongoing support to empower your health and confidence.",
    ...
    <div className="bg-white text-slate-900 font-['Inter'] overflow-x-hidden selection:bg-[#C1121F] selection:text-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            ...
            <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.35 }} className="relative z-10">
              <motion.div variants={itemUp} className="inline-flex items-center gap-2.5 px-3.5 py-2 rounded-full bg-[#C1121F]/5 border border-[#C1121F]/10">
                ...
                <span className="text-[11px] font-['Poppins'] font-semibold uppercase tracking-[0.2em] text-[#C1121F]">Women's Health Specialist</span>
              </motion.div>
              ...
              <motion.h1 variants={itemUp} className="mt-6 font-['Poppins'] text-3xl sm:text-4xl md:text-5xl font-bold text-[#0F172A] leading-tight tracking-tight">
                Empowering <span className="text-[#C1121F]">Women</span> <br /> Through Every Stage.
              </motion.h1>

              <motion.p variants={itemUp} className="mt-4 text-sm sm:text-base text-slate-600 leading-relaxed max-w-xl">
                Dedicated care for pelvic health, postnatal recovery, and long-term vitality. We provide compassionate, evidence-based physiotherapy tailored to you.
              </motion.p>
              ...
              <motion.div variants={itemUp} className="mt-7 flex flex-wrap gap-3">
                <Link to="/book" className="group relative bg-[#C1121F] text-white px-6 py-3.5 rounded-2xl font-['Poppins'] font-semibold text-sm sm:text-base overflow-hidden transition hover:shadow-xl hover:shadow-[#C1121F]/20 active:scale-95">
                  <span className="relative z-10 inline-flex items-center gap-2">Book Private Consult <ArrowRight className="w-5 h-5" /></span>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.98, x: 18 }} whileInView={{ opacity: 1, scale: 1, x: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.9, ease }} className="relative">
              <div className="relative rounded-[2.5rem] overflow-hidden shadow-[0_40px_90px_-25px_rgba(15,23,42,0.35)] border-8 border-white bg-white">
                <div className="relative aspect-[16/12]">
                  <img src={WomenImg1} alt="Women Health Care" className="w-full h-full object-cover" />
              ...
              <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }} className="absolute -top-6 -right-3 sm:-top-8 sm:-right-8 bg-[#0F172A] px-5 py-4 rounded-2xl shadow-2xl border border-white/10 hidden md:block">
                <div className="flex items-center gap-3">
                  <Heart className="w-6 h-6 text-[#C1121F]" />
                  <div>
                    <div className="font-['Poppins'] font-bold text-white text-lg">Safe</div>
                    <p className="text-[10px] text-slate-400 uppercase tracking-widest">Environment</p>
                  </div>
                </div>
              </motion.div>
            </motion.div>
      ...
      <section id="process" className="py-16 lg:py-20 bg-[#F8FAFC]">
        <div className="mx-auto max-w-7xl px-6">
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease }} className="text-center mb-10 lg:mb-14">
            <span className="text-[11px] font-['Poppins'] font-semibold uppercase tracking-[0.2em] text-[#C1121F] block">The healing approach</span>
            <h2 className="mt-3 font-['Poppins'] text-2xl sm:text-3xl md:text-4xl font-bold text-[#0F172A] leading-tight">Your Pathway to <span className="text-[#C1121F]">Vitality</span></h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 items-center">
             <motion.div initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, ease }} className="relative">
                <div className="rounded-[2.5rem] overflow-hidden shadow-xl border-8 border-white bg-white">
                  <div className="relative aspect-[16/12]">
                    <img src={WomenImg2} alt="Women Health Session" className="w-full h-full object-cover" />
             ...
             <motion.div initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, ease }}>
                <h2 className="font-['Poppins'] text-2xl sm:text-3xl md:text-4xl font-bold text-[#0F172A] leading-tight">
                  Expert care for <br />
                  <span className="text-[#C1121F]">women's vitality.</span>
                </h2>
                <p className="mt-4 text-sm sm:text-base leading-relaxed text-slate-600">
                  We understand that women’s health needs are unique. Our clinical approach focuses on restoring core and pelvic stability through gentle, evidence-based therapy.
                </p>
                <div className="mt-7 grid grid-cols-1 sm:grid-cols-2 gap-4">
                   <div className="p-5 bg-[#F8FAFC] rounded-3xl border border-slate-100">
                      <Baby className="w-8 h-8 text-[#C1121F] mb-3" />
                      <h4 className="font-['Poppins'] text-base font-bold text-[#0F172A]">Maternal</h4>
                      <p className="mt-1 text-sm text-slate-600">Expert pre & postnatal support.</p>
                   </div>
                   <div className="p-5 bg-[#F8FAFC] rounded-3xl border border-slate-100">
                      <Zap className="w-8 h-8 text-[#FACC15] mb-3" />
                      <h4 className="font-['Poppins'] text-base font-bold text-[#0F172A]">Restorative</h4>
                      <p className="mt-1 text-sm text-slate-600">Restore core function safely.</p>
                   </div>
                </div>
             </motion.div>
          </div>
      ...
      <section className="py-16 lg:py-20 bg-[#F8FAFC]">
        <div className="mx-auto max-w-7xl px-6">
          <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease }} className="text-center mb-10 lg:mb-14">
             <h2 className="font-['Poppins'] text-2xl sm:text-3xl md:text-4xl font-bold text-[#0F172A]">Areas of <span className="text-[#C1121F]">Mastery</span></h2>
             <div className="mt-4 w-20 h-1.5 bg-[#FACC15] mx-auto rounded-full" />
          </motion.div>
      ...
      <section className="pb-16 lg:pb-20 px-6">
        <motion.div initial={{ opacity: 0, scale: 0.99 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8, ease }} className="mx-auto max-w-6xl bg-[#0F172A] rounded-[3rem] p-10 sm:p-14 text-center relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10">
            <h2 className="font-['Poppins'] text-2xl sm:text-3xl md:text-4xl font-bold text-white leading-tight">Empower your <span className="text-[#C1121F]">vitality</span>.</h2>
            <p className="mt-4 text-sm sm:text-base text-slate-300 max-w-2xl mx-auto leading-relaxed">
              Experience care that is as unique as you are. We are here to support your health journey at every life stage.
            </p>
